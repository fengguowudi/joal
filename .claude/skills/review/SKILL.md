...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the project's issue tracker if available.
2. A path the user passed as an argument.
3. A PRD/spec file under `.trellis/tasks/` matching the branch name or feature.
...
The primary standards source for this project is `.trellis/spec/`. Additionally check:

- `.trellis/spec/` (all layer and guide files — the authoritative coding standards)
- `CONTEXT.md`, `CONTEXT-MAP.md`, per-context `CONTEXT.md` files
- `docs/adr/` (architectural decisions are standards)
- `.editorconfig`, `rustfmt.toml`, `clippy.toml`, `Cargo.toml` lint sections (machine-enforced standards — note them but don't re-check what tooling already checks)
