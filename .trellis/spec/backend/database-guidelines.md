...
- **Response handler chain order (fixed)**: `AnnounceEventPublisher → AnnounceReEnqueuer → BandwidthDispatcherNotifier → ClientNotifier → MergerPokeNotifier` (Rust; Java has the first four without the merger tail). This order is load-bearing:
  ...
  - `MergerPokeNotifier` is the Rust-only terminal handler: it runs **after** every other handler so the `AnnouncerFacade` fields it triggers on (`last_known_interval`, `consecutive_fails`, `last_announced_at`) already reflect the just-completed round-trip when the merger rebuilds the snapshot.
- **MergerPoke is fire-and-forget, never load-bearing**: every poke producer (`BandwidthDispatcher::recompute_speeds`, `MergerPokeNotifier::on_response`) uses `try_send` and swallows the `TrySendError`. Rationale: the merger rebuilds from scratch on every wake-up, so a dropped poke just means the *next* poke (or the next broadcast-bus transition) publishes the same frame. Never promote a poke failure to a user-visible error.
...
- Integration: `tests/seed_manager_snapshot.rs` — boot `SeedManager` against an on-disk `joal-conf/` fixture; assert `snapshot()` reports the active client filename; drop a `.torrent` and wait for the snapshot frame to include it with `last_known_interval == Some(tracker_interval)`; call `stop()` and assert `GlobalSeedStopped` lands on the broadcast bus.
